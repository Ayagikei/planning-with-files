...
description: "Use for complex multi-step planning that needs persistent file tracking or context-reset recovery."
user-invocable: true
...
- **ملفات التخطيط الخاصة بك** توضع في موقع مستندات التخطيط الموجود في المشروع عند توفره، مثل `docs/plans` أو `docs/plan` أو `docs/planning` أو مجلد docs خاص بالميزة. استخدم جذر المشروع فقط للتوافق القديم أو عندما يعتمد المشروع ذلك صراحة.

| الموقع | المحتوى المخزن |
|------|---------|
| دليل المهارة (`${CLAUDE_PLUGIN_ROOT}/`) | القوالب، النصوص البرمجية، المراجع |
| موقع مستندات التخطيط في المشروع | `task_plan.md`، `findings.md`، `progress.md` |

## البدء السريع

قبل مهمة تستفيد فعلاً من التخطيط المستمر:

1. **صنّف المهمة أولاً** — trivial / medium-light / large
2. **تجاوز المهام البسيطة** إلا إذا طلب المستخدم تتبعاً مستمراً صراحة
3. **ابحث عن موقع التخطيط** — فضّل `docs/plans` أو `docs/plan` أو `docs/planning` أو عرف المشروع المكافئ
4. **أنشئ `task_plan.md` و `findings.md` و `progress.md`** في ذلك الموقع
5. **أعد قراءة الخطة قبل القرارات** — حدّث الأهداف في نافذة الانتباه
6. **حدّث بعد كل مرحلة** — علّم المكتمل، سجّل الأخطاء

> **ملاحظة:** ملفات التخطيط توضع في موقع مستندات التخطيط في المشروع، وليس في دليل تثبيت المهارة.
...
### 1. أنشئ الخطة أولاً
لا تبدأ مهمة كبيرة أو طويلة أو كثيفة البحث أو معرضة لفقدان السياق بدون `task_plan.md`. أبق المهام الصغيرة خفيفة.
